...
async def start_deep_link(
    ...
    await message.answer(greeting(user), reply_markup=kb.main_menu(user.is_admin))
...
async def main_menu(call: CallbackQuery, user: User, state: FSMContext) -> None:
    await state.clear()
    await call.message.edit_text(greeting(user), reply_markup=kb.main_menu(user.is_admin))
    await call.answer()
...
async def show_balance(call: CallbackQuery, user: User, state: FSMContext) -> None:
    await state.clear()
    await call.message.edit_text(balance_text(user), reply_markup=kb.balance_menu())
    await call.answer()


@router.callback_query(MenuCB.filter(F.action == "rules"))
async def show_rules(call: CallbackQuery) -> None:
    await call.message.edit_text(settings.get("text_rules"), reply_markup=kb.back_only())
    await call.answer()
...
async def to_deposit_menu(call: CallbackQuery, session: AsyncSession, user: User, state: FSMContext) -> None:
